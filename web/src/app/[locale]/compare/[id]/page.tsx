...
export default async function ComparePage({ params: { id } }: ComparePageProps) {
  const people: Person[] = base64url.decode(id)
  const reports = await Promise.all(people.map(async (person) => {
  ...
  const getNamedFacets = (domain: string) => reports
    .map((report) => {
      const domainResult = report.report.results.find((result) => result.domain === domain)
      return {
        name: report.name,
